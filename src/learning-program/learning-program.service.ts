...
export class LearningProgramService {
    ...
    async create(learningProgram: LearningProgram, user: User): Promise<LearningProgram> {
    ...
    async findAll(user: User): Promise<LearningProgram[]> {
      return await this.learningProgramRepository.findBy({ user } as FindOptionsWhere<LearningProgram>);
    }
  
    async findOne(id: string, user: User): Promise<LearningProgram> {
      return await this.learningProgramRepository.findOneBy({ user, id } as FindOptionsWhere<LearningProgram>);
    }
  
    async update(learningProgram: LearningProgram, user: User): Promise<UpdateResult> {
      const learningProgramToUpdate: unknown = await this.findOne(learningProgram.id, user);
      if (!learningProgramToUpdate) {
        throw new UnauthorizedException(
          'You are not elligible to update this program',
        );
      }
      return await this.learningProgramRepository.update(learningProgramToUpdate as FindOptionsWhere<LearningProgram>, learningProgram);
    }
    ...
    async remove(id: string, user: User): Promise<DeleteResult> {
      ...
      return await this.learningProgramRepository.delete(learningProgramToDelete as FindOptionsWhere<LearningProgram>);
    }
}
